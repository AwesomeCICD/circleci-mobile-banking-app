...
Each `run-trial.sh` invocation opens a **draft PR** on `bench/<arm>-<trial>` after the **first commit** on the trial branch (GitHub requires a diff from `main`). Commits and CI runs accumulate on the PR as the agent works. When the trial finishes with **green CI**, `trial-pr.sh finalize` marks the PR **ready for review** and updates the body with run metrics. Links appear in `report-phase2.md` per-trial table.
