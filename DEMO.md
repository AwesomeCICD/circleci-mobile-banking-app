...
## What's the "Stop hook"?

A **Stop hook** is a built-in Claude Code feature: a shell command you can configure to run automatically every time the agent finishes a turn. It lives in `.claude/settings.json`. In this repo it looks like this:
...
You don't write this file by hand — **`chunk init` generates it for you** when you set up a project. The file's first line literally says `"_comment": "Generated by chunk init."`

### What it does in this demo

Every time Claude finishes a reply, Claude Code runs `chunk validate` before returning control to you. If validation **fails**, the failure output is injected back into the conversation as a system message and Claude's *next* turn begins with that error context already loaded. The agent reacts to the failure exactly the way a developer would react to a CI red build — except the failure arrived in seconds, not minutes, and never reached CI.

That's the entire trick. You never type `chunk validate`. You never ask Claude to run it. The hook fires between every message — that's why the validation output lands *between* the three messages you send on stage.

```
You type:       Quick sanity check...
Claude replies: Looks fine on a read-through.
   ↓
   ↓  [Stop hook auto-fires → chunk validate runs → lint-payments fails]
   ↓
Claude:         Validation failed — let me remove the unused import.
You type:       go ahead
   ↓
   ↓  [Stop hook fires again → test-payments fails]
   ↓
Claude:         Test asserts old text — let me update it.
You type:       go ahead
   ↓
   ↓  [Stop hook fires → all 8 gates green]
   ↓
Claude:         Validated. Ready to push.
```
